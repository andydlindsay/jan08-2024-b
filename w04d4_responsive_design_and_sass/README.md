...
* by default, the height of a box is the minimum that contain the content in it. in order for
percentages to work for children, the parent container must have an explicit height defined.
...
/* vh and vw */
p.viewport {
  /* 25% of the viewport width    1 vw is a 1/100th of the viewport */
  width: 25vw; 
  height: 50vh;
  font-size: 10vh;
}

/* em (always relative the FONT SIZE of the parent) 
  and rem (not relative the to the immediate parent, 
  instead relative to html tag (default 16px)) 
  good for padding, font-sizes, etc. */
p.relative {
  width: 25em;
  height: 40rem;
  border-width: 2em;
...
Note that & is used like 'this' so that you can use pseudo-classes.
